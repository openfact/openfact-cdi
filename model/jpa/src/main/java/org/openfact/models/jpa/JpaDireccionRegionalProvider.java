...
import org.openfact.models.jpa.entities.DireccionRegionalEntity;
...
public class JpaDireccionRegionalProvider extends AbstractHibernateStorage implements DireccionRegionalProvider {
	...
	public DireccionRegionalModel findById(String id) {
		...
		return direccionRegionalEntity != null ? new DireccionRegionalAdapter(em, direccionRegionalEntity) : null;
	}

	@Override
	public DireccionRegionalModel findByDenominacion(String denominacion) {				
		TypedQuery<DireccionRegionalEntity> query = em.createNamedQuery("DireccionRegionalEntity.findByDenominacion",
		...
		if (results.isEmpty()) {
			...
			throw new IllegalStateException(
					"Mas de un DireccionRegionalEntity con denominacion=" + denominacion + ", results=" + results);
		} else {
	...
	public SearchResultsModel<DireccionRegionalModel> search(SearchCriteriaModel criteria) {
		SearchResultsModel<DireccionRegionalEntity> entityResult = find(criteria, DireccionRegionalEntity.class);
	...
	@Override
	public SearchResultsModel<DireccionRegionalModel> search(SearchCriteriaModel criteria, String filterText) {
		SearchResultsModel<DireccionRegionalEntity> entityResult = findFullText(criteria, DireccionRegionalEntity.class,
